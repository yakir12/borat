...
using PyPlot # would be nice to not need to load this on every processor
@everywhere using Shapes, RayTrace, RefractiveIndexGradients # PyPlot is slow and in the ultimate version will be removed
@everywhere begin
# constants:
...
const nrays = 10 # the number of discrete rays
...
ri(r::Float64) = sqrt(max(0, 2. - r*r/lens_r2)) # this is the refractive index gradient function. I set this RIG to be equal to the Luneburg lens, so that we can check the results real quick.
...
@everywhere function oneIter(i::Int64)
